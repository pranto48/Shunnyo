...
export default function MessageBubble({ message, isGroup = false }) {
  ...
  const isSoloEmoji = isOnlyEmojiMessage(message.content) && !message.attachment && !message.audioDuration && !message.isDeleted;
    ...
    >
      ...
      <div className="relative max-w-[85%] sm:max-w-[70%] md:max-w-[60%] flex items-end gap-1.5">
        ...
        >
          ...
          {message.isDeleted ? (
            <div className="flex items-center space-x-2 py-1 text-slate-400 italic text-xs select-none">
              <Trash2 className="w-3.5 h-3.5 opacity-50 flex-shrink-0" />
            ...
            <>
              ...
              {!isSoloEmoji && (
                ...
                >
                  {/* Edited indicator */}
                  {message.isEdited && (
                    <span className="text-[9px] opacity-75 font-medium mr-0.5">
                      (সম্পাদিত)
                    </span>
                  )}
